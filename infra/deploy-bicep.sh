...
if [ "$GREENFIELD" = true ]; then
    echo "=== REMAINING MANUAL STEPS (greenfield only) ==="
    echo ""
    echo "1. Create SQL managed identity user:"
    echo "   Connect to mi-${ENV}-sql.database.windows.net / mi-${ENV} and run:"
    echo ""
    ...
    echo ""
    echo "2. Run database schema and migrations:"
    echo "   Execute schema.sql, then server/migrations/*.sql in order"
    echo ""
    echo "3. Create client token:"
    echo "   cd server && uv run python scripts/manage_tokens.py create --env ${ENV}"
    echo ""
    echo "4. Update parameter file with CORS origin (for future deploys):"
    echo "   Edit ${PARAM_FILE}:"
    echo "   param corsOrigins = 'https://${FQDN}'"
    echo ""
    echo "5. Grant admin consent (requires Global Admin):"
