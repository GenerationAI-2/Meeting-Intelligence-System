...
# Workspace slug (name column in control DB) — override with WORKSPACE_NAME env var if needed
WORKSPACE_NAME="${WORKSPACE_NAME:-general}"
...
SEED_SQL="IF NOT EXISTS (SELECT 1 FROM workspaces WHERE name = '${WORKSPACE_NAME}')
BEGIN
    INSERT INTO workspaces (name, display_name, db_name, is_default, created_by)
    VALUES ('${WORKSPACE_NAME}', '${WORKSPACE_DISPLAY_NAME}', '${SQL_DATABASE}', 1, '${DEPLOYER_EMAIL}');
    PRINT '${WORKSPACE_DISPLAY_NAME} workspace seeded';
END
ELSE
    PRINT '${WORKSPACE_DISPLAY_NAME} workspace already exists -- skipping';"
...
END"
    ...
    WHERE u.email = '${DEPLOYER_EMAIL}' AND w.name = '${WORKSPACE_NAME}'
)
BEGIN
    ...
    PRINT 'Deployer added as chair on ${WORKSPACE_DISPLAY_NAME} workspace';
END
ELSE
    PRINT 'Deployer already a member of ${WORKSPACE_DISPLAY_NAME} workspace -- skipping';"
...
if [ "$DB_INIT_OK" = true ]; then
    ...
    if [ "$TOKEN_OK" = true ]; then
        ...
        echo "    uv run python scripts/manage_tokens.py create --user '$PRIMARY_EMAIL' --workspace $WORKSPACE_NAME --role chair"
    fi
else
...
if [ "$DB_INIT_OK" = false ]; then
    ...
    echo ""
    echo "5. Seed workspace:"
    echo "   sqlcmd -S $SQL_SERVER -d $CONTROL_DATABASE -G --authentication-method=ActiveDirectoryDefault"
    echo "   > INSERT INTO workspaces (name, display_name, db_name, is_default, created_by)"
    echo "     VALUES ('$WORKSPACE_NAME', '$WORKSPACE_DISPLAY_NAME', '$SQL_DATABASE', 1, '$DEPLOYER_EMAIL');"
    echo ""
fi
