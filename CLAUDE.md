...
| Commented `subscriptionId` in bicepparam causes wrong-subscription deploy | Resources (RG, SQL, Container App, KV) created in MyAdvisor CSP instead of client's subscription | `// param subscriptionId = '...'` is still matched by grep+sed, but deploy-bicep.sh's ACR cross-subscription search switches `az account set` to MyAdvisor and never switches back. The extracted subscription ID from the commented line is empty, so `deploy-new-client.sh` doesn't call `az account set`. | Ensure `deploy-new-client.sh` extracts the subscription ID correctly (it does — grep matches commented lines). The real fix: deployer must `az account set` to client subscription before running, or uncomment the subscriptionId line (but it's not a real Bicep param so Bicep validation fails). Current approach: leave commented, rely on deploy-new-client.sh parsing. | 2026-03-12 |
| Key Vault soft-delete + purge protection blocks reuse of KV name for 90 days | Bicep deploy fails with `VaultAlreadyExists` when creating KV with same name in different subscription | Deleted resource group in wrong subscription, but KV enters soft-deleted state with purge protection enabled. Purge protection can't be disabled once enabled. KV name is globally unique and blocked for 90 days. | Added `keyVaultNameOverride` param to `main.bicep`. Victory Knives uses `kv-mi-prod-vknives2`. | 2026-03-12 |
...
| Victory Knives | ca-mi-prod-vknives.happysmoke-3c7c6e93.australiaeast.azurecontainerapps.io | sqldb-mi-prod-vknives + sqldb-mi-prod-vknives-control | 0-10 | Client (Gareth). Subscription `98eaa078`. KV: `kv-mi-prod-vknives2` (name override due to soft-delete collision). |
